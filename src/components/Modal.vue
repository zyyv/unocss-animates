...
<template>
  <div v-show="modalVisible" bg="#222 op-80" pf z-10 h-screen w-screen fcc>
    <div pr fccc gap-1 class="prose">
      <div i-ri:close-circle-line pa right-0 top-0 cursor-pointer text-xl hover-i-ri:close-circle-fill
        @click="handleCloseModal()" />
      <h2 mb-2 mt-0 px-4 py-2 text-3xl text-red fw-600 hover="b b-red-300 b-dashed rd-2 cursor-pointer"
        @click="handleCopy(activeAnimate.name)">
        {{ activeAnimate.name }}
      </h2>
      <p>
        Uno Class:
        <span hover="b b-teal-300 b-dashed rd-2 cursor-pointer" inline-block px-2 text-teal
